...
class EnhancedFinancialResearcher:
    ...
    def parse_timeframe_from_prompt(self, user_prompt: str) -> int:
        """
        Parse timeframe from user prompt and update if found
        
        Args:
            user_prompt (str): User's input prompt
            
        Returns:
            int: Detected timeframe in days
            
        Examples of prompts it can parse:
        - "Search for AAPL data from last 7 days"
        - "Give me Tesla news from past 2 weeks" 
        - "Find Microsoft info from last 3 months"
        - "Show me Apple data from yesterday only"
        """
        import re
        ...
        # Pattern matching for different timeframe formats
        patterns = [
            (r'last (\d+) days?', 1),
            (r'past (\d+) days?', 1),
            (r'(\d+) days?', 1),
            ...
            (r'past (\d+) months?', 30),
            (r'(\d+) months?', 30),
            (r'yesterday', None),  # Special case
            (r'today', None),     # Special case
        ]
        ...
        for pattern, multiplier in patterns:
            ...
            if match:
                if pattern in ['yesterday', 'today']:
                    ...
                    detected_days = number * multiplier
                
                # Update timeframe if different from current
                if detected_days != self.timeframe_days:
                    self.set_timeframe(detected_days)
                
                return detected_days
        
        # No timeframe found, keep current
        return self.timeframe_days
    
    def search(self, query: str) -> str:
        """
        Intelligent search that automatically detects and applies timeframe from query
        ...
        """
        # Parse timeframe from the query
        detected_timeframe = self.parse_timeframe_from_prompt(query)
        
        # Return search info (the actual search will be done by the agent)
        return f"🔍 Searching with {detected_timeframe}-day timeframe: {query}"
